...
// MODIFY BY SHUZHOU XIE
var rImages = /([\s\S]*?)(url\(([^)]+)\)[\s\S]*?[;}])(?!\s*[;,]?\s*\/\*\s*ImageEmbed:skip\s*\*\/)|([\s\S]+)/img;
      ...
      if(group[4] == null) {
        ...
        if(cache[img]) {
          ...
          exports.image(loc, opts, function(err, resp, cacheable) {
            if (err == null) {
              // var url = "url(" + resp + ")";
              // MODIFY BY SHUZHOU XIE, only support for background-image
              var cssText = group[2];
              var lastChar = cssText.substring(cssText.length - 1);
              cssText = cssText.substring(0, cssText.length - 1);
              var url = cssText.replace(group[3], resp);
              url += (';*background-image:url(' + group[3] + ')' + lastChar);
              result += url;
